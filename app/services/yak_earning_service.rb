...
class YakEarningService
  ...
  def self.award(user:, action_key:, related_post: nil, related_topic: nil, event_id: nil)
    ...
    wallet.with_lock do
      if already_awarded?(wallet, action_key, idempotency_key)
        Rails.logger.info("[Yaks] Award skipped: Event already rewarded (#{idempotency_key})")
        return false
      end
  ...
  def self.earning_idempotency_key(action_key, event_id:, related_post:, related_topic:)
    event_id ||= action_key == "topic_created" ? related_topic&.id : related_post&.id
